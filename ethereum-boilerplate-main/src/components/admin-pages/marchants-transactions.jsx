...
import SingleMarchantTransaction from "./single-marchant-transactions";
import baseUrl from "api/base-url";
import Spinner from "components/spinner/spinner.component";

export default function MarchantTransactions() {
    ...
    const fetchUserTransactions = async () => {
      ...
      try {
        const fetchResult = await fetch(
          `${baseUrl()}/get-all-marchants-transactions`,
          transactionsOptions,
        );
        if (!fetchResult.ok) {
          setLoading(false);
  ...
  const searchByTransactionId = async () => {
    ...
    try {
      ...
      if (!fetchResult.ok) {
        setLoading(false);
        setNewErr(
          "Error occured while fetching marchants transactions. Please try again",
        );
        return;
      }
      const data = await fetchResult.json();
  ...
  return (
    <div className="absolute top-20 left-0 md:left-80 bg-zinc-200 md:w-120 w-screen p-5 rounded-xl">
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-6">
          <input
            ...
            name="walletAddress"
            type="text"
            placeholder="Search for Transactions By Marchant ID"
            required
          ></input>
          <button
